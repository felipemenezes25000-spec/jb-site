 ...
 */
export async function AcessoDaConta() {
 ...
 */
export function AcessoDaContaEsqueleto() {
  return (
    <Link href="/entrar" aria-label="Área da Clínica" className={CAIXA_CONTA}>
      <span aria-hidden className={AVATAR}>
        <User className="size-4" />
      </span>
      <span className="hidden max-w-36 text-left leading-tight lg:block">
        <span aria-hidden className="mt-0.5 block h-3 w-14 rounded bg-graf-200" />
        <span className="mt-1 block text-sm font-bold text-graf-900">Área da Clínica</span>
      </span>
    </Link>
  );
}
 ...
 */
export function ContadorDoCarrinhoEsqueleto() {
  return (
