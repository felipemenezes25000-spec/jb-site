...
export function LinhasCarrinho({ linhas }: { linhas: LinhaCarrinho[] }) {
  ...
  const transicaoLayout = movimentoReduzido
    ? { duration: 0 }
    : { type: "spring" as const, stiffness: 430, damping: 38, mass: 0.72 };

  return (
    <motion.ul
      layout={!movimentoReduzido}
      data-motion-cart-list
      className="overflow-hidden rounded-xl border border-graf-200 bg-white"
    >
        ...
        {linhas.map((linha, indice) => {
          ...
          return (
            <motion.li
              layout={!movimentoReduzido}
              key={linha.id}
              data-motion-cart-item
              initial={movimentoReduzido ? false : { opacity: 0, y: 10, scale: 0.992 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={movimentoReduzido ? { opacity: 0 } : { opacity: 0, x: 26, scale: 0.975 }}
              transition={transicaoLayout}
              className={cn(
                "p-4 sm:p-5",
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-3.5">
                  <div className="flex flex-wrap items-start justify-between gap-x-5 gap-y-2">
                    <div className="min-w-0 flex-1 basis-44">
                      {linha.marca ? <p className="micro text-graf-500">{linha.marca}</p> : null}
                    ...
                    <motion.div layout="position" className="shrink-0 text-right">
                      {linha.disponivel ? (
                        <>
                          <motion.p
                            key={linha.totalCents}
                            initial={movimentoReduzido ? false : { opacity: 0.45, y: 3 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: movimentoReduzido ? 0 : 0.2 }}
                            className="text-lg font-extrabold tabular leading-none text-graf-950"
                          >
                      ...
                      {linha.unico ? (
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          <div className="flex items-center rounded-lg border border-graf-300 bg-white">
                            ...
                            <span
                              aria-live="polite"
                              className="relative flex h-11 w-10 items-center justify-center overflow-hidden border-x border-graf-200 text-center text-sm font-bold tabular text-graf-950"
                            >
                              <span className="sr-only">Quantidade de {linha.nome}: {linha.quantidade}</span>
                              <AnimatePresence initial={false} mode="popLayout">
                                <motion.span
                                  aria-hidden
                                  key={linha.quantidade}
                                  initial={movimentoReduzido ? false : { opacity: 0, y: linha.quantidade > 1 ? 6 : -6 }}
                                  animate={{ opacity: 1, y: 0 }}
                                  exit={movimentoReduzido ? { opacity: 0 } : { opacity: 0, y: -5 }}
