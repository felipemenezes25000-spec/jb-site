...
export async function SecaoCategorias() {
  ...
  return (
    /* Ritmo da home: lista é densa, narrativa respira.
       ...
       carrega a informação no próprio desenho e fica no respiro `sm`;
       assistência e fechamento, que são argumento e não vitrine, vão ao `md`.
       Os dois degraus agora vêm da escala de `Secao`, e não de um par de
       classes escrito em cada faixa. */
    <Secao fundo="branco" largura="loja" espaco="sm" separador>
      <TituloSecao
        sobretitulo="Categorias"
        titulo="Encontre pelo que a clínica precisa"
        descricao="Entre por categoria para comparar produtos semelhantes com menos ruído."
            ...
            <li key={categoria.slug}>
              ...
              >
                <span
                  ...
                  className="relative flex aspect-[4/3] items-center justify-center overflow-hidden border-b border-graf-100 bg-surface-muted"
                >
                  {foto ? (
                    <Image
                      data-imagem-produto
