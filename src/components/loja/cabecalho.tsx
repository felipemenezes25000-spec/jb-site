                    ...
                    >
                      <IconeMenu
                        ...
                        className={cn(
                          ...
                          aberto || estaAtivo
                            ? "text-jb-600"
                            : "text-graf-500 group-hover:text-jb-600",
                        )}
                      />
...
function PainelMega({
  ...
  if (chave === "catalogo") {
    return <MegaCatalogo categorias={categorias} condicoes={condicoes} />;
  }
...
function TituloMega({
  ...
  return (
    <div>
      <p className="text-[0.68rem] font-extrabold uppercase tracking-[0.14em] text-jb-600">
        {etiqueta}
      </p>
      <h2 className="mt-2 text-[1.35rem] font-extrabold tracking-[-0.03em] text-graf-950">
        {titulo}
      </h2>
      {descricao ? (
        <p className="mt-1.5 max-w-[48rem] text-sm leading-relaxed text-graf-500">
          {descricao}
        </p>
      ) : null}
    </div>
    ...
    <div className="grid gap-0 xl:grid-cols-[1.7fr_0.8fr_0.9fr]">
      <section className="p-7 pr-8">
        <div className="flex items-start justify-between gap-6">
          <TituloMega
            etiqueta="Categorias"
            titulo="Encontre pelo tipo de equipamento"
            descricao="Navegue pelas categorias da loja sem sair do contexto da clínica."
          />
          <Link
            href="/loja"
            className="mt-1 inline-flex shrink-0 items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-extrabold text-jb-700 transition hover:bg-jb-50 hover:text-jb-900"
          >
        ...
        {destaques.length ? (
              ...
              <li key={categoria.slug}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-[0.92rem] font-extrabold leading-tight text-graf-900 transition-colors group-hover:text-jb-700">
                      {categoria.name}
                    </span>
                    <span className="mt-1 block text-xs text-graf-500">
                      Ver equipamentos desta categoria
                    </span>
                  </span>
                  {categoria.count > 0 ? (
                    <span className="tabular rounded-full bg-white px-2.5 py-1 text-[0.7rem] font-bold text-graf-500 ring-1 ring-graf-200">
                      {categoria.count}
                    </span>
                  ) : null}
                  <ArrowRight
                    className="size-4 shrink-0 text-graf-400 transition group-hover:translate-x-0.5 group-hover:text-jb-600"
                    aria-hidden
                  />
                </Link>
              </li>
          ...
          <div className="mt-5 rounded-2xl border border-dashed border-jb-200 bg-jb-50/50 p-6 text-sm text-graf-600">
      ...
      <section className="border-l border-graf-200/80 p-7">
        ...
        <ul className="mt-5 space-y-2.5">
          {condicoes.slice(0, 4).map((condicao, indice) => (
            <li key={condicao.slug}>
              <Link
                href={`/${condicao.slug}`}
                className="group flex items-center gap-3 rounded-2xl border border-graf-200/80 p-3.5 transition hover:border-jb-200 hover:bg-jb-50/50"
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-extrabold text-graf-900 group-hover:text-jb-700">
                    {condicao.rotulo}
                  </span>
                  {condicao.total > 0 ? (
                    <span className="mt-0.5 block text-xs text-graf-500">
                      {condicao.total} disponíveis
                    </span>
                  ) : (
                    <span className="mt-0.5 block text-xs text-graf-500">
                      Explorar seleção
        ...
        <div className="mt-5 grid gap-2 border-t border-graf-200 pt-4">
          <Link
            href="/marcas"
            className="group flex min-h-11 items-center gap-3 rounded-xl px-2 text-sm font-bold text-graf-700 hover:bg-graf-50 hover:text-jb-700"
          >
            <Tag className="size-4 text-jb-600" aria-hidden /> Marcas
            <ArrowRight className="ml-auto size-3.5 opacity-50 group-hover:opacity-100" aria-hidden />
          </Link>
          <Link
            href="/pecas-e-acessorios"
            className="group flex min-h-11 items-center gap-3 rounded-xl px-2 text-sm font-bold text-graf-700 hover:bg-graf-50 hover:text-jb-700"
          >
            <Settings className="size-4 text-jb-600" aria-hidden /> Peças e acessórios
            <ArrowRight className="ml-auto size-3.5 opacity-50 group-hover:opacity-100" aria-hidden />
          </Link>
        </div>
      </section>
      ...
      <aside className="m-4 ml-0 flex min-h-[360px] flex-col overflow-hidden rounded-[1.4rem] bg-gradient-to-br from-jb-50 via-white to-jb-100/60 p-6 ring-1 ring-jb-100">
        <div className="relative flex flex-1 flex-col">
          <div
            className="absolute -right-12 -top-10 size-44 rounded-full bg-jb-200/35 blur-2xl"
            aria-hidden
          />
          <span className="relative inline-flex w-fit items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-[0.68rem] font-extrabold uppercase tracking-[0.11em] text-jb-700 shadow-sm ring-1 ring-jb-100">
            <Sparkles className="size-3.5" aria-hidden /> Tecnologia para a clínica
          </span>
          <h3 className="relative mt-5 max-w-[18rem] text-[1.8rem] font-extrabold leading-[1.04] tracking-[-0.045em] text-graf-950">
            Equipamentos que acompanham o seu dia a dia.
          </h3>
          <p className="relative mt-3 text-sm leading-relaxed text-graf-600">
            Compare opções, condições e categorias antes de decidir.
          </p>
          <div className="relative mt-auto pt-6">
            <Link
              href="/loja"
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-jb-600 px-4 text-sm font-extrabold text-white shadow-[0_10px_24px_rgba(196,14,21,0.2)] transition hover:bg-jb-700"
            >
  ...
  return (
    <div className="grid gap-0 xl:grid-cols-[1.1fr_1.2fr_0.8fr]">
      <section className="p-7">
        <TituloMega
          etiqueta="Seminovos JB"
          titulo="Uma seleção com contexto técnico"
          descricao="Entre direto na vitrine de seminovos e compare alternativas por categoria."
        />
        <div className="mt-6 rounded-[1.4rem] border border-graf-200 bg-white p-6 text-graf-950">
          <RefreshCw className="size-6 text-jb-600" aria-hidden />
          <h3 className="mt-5 text-xl font-extrabold tracking-[-0.03em] text-graf-950">
            Compare antes de escolher.
          </h3>
          <p className="mt-2 text-sm leading-relaxed text-graf-600">
            Use a comparação e o simulador de custo para enxergar melhor as alternativas disponíveis.
          </p>
          <Link
            href="/seminovos"
            className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl border border-graf-200 bg-white px-4 text-sm font-extrabold text-graf-950 transition hover:border-jb-200 hover:bg-jb-50 hover:text-jb-700"
          >
            Ver seminovos <ArrowRight className="size-4" aria-hidden />
          </Link>
        </div>
      </section>
        ...
        <ul className="mt-5 grid gap-3 sm:grid-cols-2">
          {categorias.slice(0, 6).map((categoria) => (
            <li key={categoria.slug}>
              <Link
                href={`/categoria/${categoria.slug}`}
                className="group flex min-h-16 items-center gap-3 rounded-2xl border border-graf-200/80 p-3.5 transition hover:border-jb-200 hover:bg-jb-50/40"
              >
                <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-graf-50 text-jb-600">
                  <Package className="size-4" aria-hidden />
                </span>
                <span className="min-w-0 flex-1 text-sm font-extrabold text-graf-900 group-hover:text-jb-700">
                  {categoria.name}
                </span>
      ...
      <aside className="border-l border-graf-200/80 p-7">
        ...
        <div className="mt-5 space-y-3">
          <AtalhoCard
            ...
            descricao="Coloque opções lado a lado."
          />
          <AtalhoCard
            href="/simulador-de-custo"
            icone={<RefreshCw className="size-4" aria-hidden />}
            titulo="Reparar, seminovo ou novo"
            descricao="Abra o simulador de custo."
          />
          <AtalhoCard
            href="/marcas"
            icone={<Tag className="size-4" aria-hidden />}
            titulo="Ver marcas"
            descricao="Navegue pela marca do equipamento."
          />
        </div>
      </aside>
    </div>
    ...
    <div className={cn("grid gap-0", temContato && "xl:grid-cols-[1.6fr_0.75fr]")}>
      <section className="p-7">
        <TituloMega
          ...
          titulo="Do chamado ao acompanhamento, tudo no mesmo lugar"
          descricao="Escolha o caminho certo para abrir atendimento, entender o fluxo ou cuidar da manutenção preventiva."
        />
        <ul className="mt-5 grid gap-3 sm:grid-cols-2">
          {MENU_ASSISTENCIA.map((item, indice) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className="group flex min-h-[108px] items-start gap-4 rounded-2xl border border-graf-200/80 bg-white p-4 transition-all hover:-translate-y-0.5 hover:border-jb-200 hover:shadow-[0_10px_24px_rgba(26,28,30,0.07)]"
              >
                <span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-jb-50 text-jb-600">
                  {indice === 0 ? (
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-extrabold text-graf-950 group-hover:text-jb-700">
                    {item.rotulo}
                  </span>
                  {item.descricao ? (
                    <span className="mt-1.5 block text-xs leading-relaxed text-graf-500">
                      {item.descricao}
                    </span>
                  ) : null}
                </span>
      ...
      {temContato ? (
        <aside className="m-4 ml-0 flex flex-col rounded-[1.4rem] bg-gradient-to-br from-jb-700 to-jb-950 p-6 text-white shadow-[0_16px_34px_rgba(111,15,19,0.2)]">
          ...
          <h3 className="mt-5 text-xl font-extrabold tracking-[-0.03em] text-white">
            Prefere falar com a equipe?
          </h3>
          <p className="mt-2 text-sm leading-relaxed text-white/70">
            {horario ? `Equipe técnica JB. ${horario}.` : "Equipe técnica JB."}
          </p>
          <div className="mt-auto space-y-2 pt-6">
            {whatsapp ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-white px-4 text-sm font-extrabold text-jb-800 transition hover:bg-jb-50"
              >
                <MessageCircle className="size-4" aria-hidden /> WhatsApp {whatsapp}
              </a>
            ) : null}
            {telefone ? (
              <a
                href={telHref(telefone)}
                className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/25 px-4 text-sm font-extrabold text-white transition hover:bg-white/10"
              >
  ...
  return (
    <div className="grid gap-0 xl:grid-cols-[1.4fr_0.9fr]">
      <section className="p-7">
        <TituloMega
          etiqueta="Manutenção"
          titulo="Organize o cuidado antes da parada"
          descricao="Acesse manutenção preventiva, planos, serviços e orçamento sem procurar em várias páginas."
        />
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <AtalhoCard
            href="/manutencao-preventiva"
            icone={<Settings className="size-4" aria-hidden />}
            titulo="Manutenção preventiva"
            descricao="Entenda o serviço e como funciona."
          />
          <AtalhoCard
            href="/planos-de-manutencao"
            icone={<ShieldCheck className="size-4" aria-hidden />}
            titulo="Planos de manutenção"
            descricao="Veja as opções de acompanhamento."
          />
          <AtalhoCard
            href="/servicos"
          ...
          <AtalhoCard
            ...
            descricao="Leve sua necessidade para a equipe JB."
          />
        </div>
      </section>
      <aside className="m-4 ml-0 overflow-hidden rounded-[1.4rem] border border-jb-100 bg-gradient-to-br from-jb-50 via-white to-graf-50 p-6">
        <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-[0.7rem] font-extrabold uppercase tracking-[0.12em] text-jb-700 ring-1 ring-jb-100">
          <ShieldCheck className="size-3.5" aria-hidden /> Cuidado contínuo
        </span>
        <h3 className="mt-5 max-w-[22rem] text-2xl font-extrabold tracking-[-0.035em] text-graf-950">
          Uma rota clara para manter a operação organizada.
        </h3>
        <p className="mt-3 max-w-[28rem] text-sm leading-relaxed text-graf-600">
          Centralize os próximos passos e encontre rapidamente o serviço mais adequado ao equipamento.
        </p>
        <Link
          href="/manutencao-preventiva"
          className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-xl bg-jb-600 px-4 text-sm font-extrabold text-white transition hover:bg-jb-700"
        >
          Conhecer manutenção preventiva <ArrowRight className="size-4" aria-hidden />
        </Link>
      </aside>
    </div>
  );
...
function MegaCentral() {
  return (
    <div className="grid gap-0 xl:grid-cols-[1.35fr_0.95fr]">
      <section className="p-7">
        <TituloMega
          etiqueta="Central Técnica"
          titulo="Conteúdo técnico sem cara de menu institucional"
          descricao="Acesse aprendizados da bancada, casos, dúvidas e estrutura a partir de um só ponto."
        />
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <AtalhoCard
            ...
            descricao="Artigos e conteúdo da equipe."
          />
          <AtalhoCard
            href="/cases"
            icone={<Sparkles className="size-4" aria-hidden />}
            titulo="Cases técnicos"
            descricao="Veja casos e experiências publicadas."
          />
          <AtalhoCard
            href="/faq"
            icone={<MessageCircle className="size-4" aria-hidden />}
            titulo="Dúvidas frequentes"
            descricao="Encontre respostas rápidas."
          />
          <AtalhoCard
            href="/estrutura"
            icone={<Settings className="size-4" aria-hidden />}
            titulo="Nossa estrutura"
            descricao="Conheça a estrutura da JB."
          />
          <AtalhoCard
            href="/sobre"
            icone={<User className="size-4" aria-hidden />}
            titulo="Sobre a JB"
            descricao="História e posicionamento."
          />
          <AtalhoCard
            href="/contato"
      ...
      <aside className="m-4 ml-0 flex flex-col rounded-[1.4rem] border border-jb-100 bg-jb-50/60 p-6">
        <span className="flex size-11 items-center justify-center rounded-2xl bg-white text-jb-600 ring-1 ring-jb-100">
          <BookOpen className="size-5" aria-hidden />
        </span>
        <h3 className="mt-5 text-2xl font-extrabold tracking-[-0.035em] text-graf-950">
          O que a bancada aprende vira referência.
        </h3>
        <p className="mt-3 text-sm leading-relaxed text-graf-600">
          Use a Central Técnica para aprofundar a decisão e entender melhor os equipamentos.
        </p>
        <Link
          href="/central-tecnica"
          className="mt-auto inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-jb-500 px-4 text-sm font-extrabold text-white transition hover:bg-jb-600"
        >
          Explorar Central Técnica <ArrowRight className="size-4" aria-hidden />
        </Link>
      </aside>
    </div>
  );
...
function AtalhoCard({
  ...
  return (
    <Link
      href={href}
      className="group flex min-h-[94px] items-start gap-3 rounded-2xl border border-graf-200/80 bg-white p-4 transition-all hover:-translate-y-0.5 hover:border-jb-200 hover:bg-jb-50/35 hover:shadow-[0_8px_20px_rgba(26,28,30,0.06)]"
    >
      <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-jb-50 text-jb-600 ring-1 ring-jb-100">
        {icone}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-sm font-extrabold leading-tight text-graf-950 group-hover:text-jb-700">
          {titulo}
        </span>
        <span className="mt-1.5 block text-xs leading-relaxed text-graf-500">
          {descricao}
      ...
      <ArrowRight
        className="mt-1 size-3.5 shrink-0 text-graf-400 transition group-hover:translate-x-0.5 group-hover:text-jb-600"
        aria-hidden
      />
    </Link>
  );
