...
export function FormAssistencia({
  ...
  return (
    <form action={acao} className="overflow-hidden rounded-2xl border border-graf-200 bg-white shadow-card">
      ...
      <div className="p-5 sm:p-7 lg:p-8">
        ...
        <div className={etapa === 1 ? "block" : "hidden"}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="sm:col-span-2"><span className="mb-2 block text-sm font-bold">Nome</span><input name="nome" required defaultValue={nome} className={input} /></label>
            <label><span className="mb-2 block text-sm font-bold">E-mail</span><span className="relative block"><Mail className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-graf-400" aria-hidden /><input name="email" type="email" required defaultValue={email} className={`${input} pl-11`} /></span></label>
            <label><span className="mb-2 block text-sm font-bold">Telefone / WhatsApp</span><span className="relative block"><Phone className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-graf-400" aria-hidden /><input name="telefone" required defaultValue={telefone} className={`${input} pl-11`} /></span></label>
          </div>
        </div>
        ...
        <div className={etapa === 3 ? "block" : "hidden"}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label><span className="mb-2 block text-sm font-bold">CEP</span><input name="cep" className={input} /></label>
            <label><span className="mb-2 block text-sm font-bold">Cidade</span><input name="cidade" className={input} /></label>
            <label className="sm:col-span-2"><span className="mb-2 block text-sm font-bold">Endereço</span><span className="relative block"><Building2 className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-graf-400" aria-hidden /><input name="endereco" className={`${input} pl-11`} /></span></label>
            ...
            <label><span className="mb-2 block text-sm font-bold">UF</span><input name="estado" maxLength={2} className={input} /></label>
        ...
        <div className="mt-8 flex items-center justify-between gap-3 border-t border-graf-100 pt-6">
          ...
          {etapa < 3 ? (
            <button type="button" onClick={() => setEtapa((v) => Math.min(3, v + 1))} className="inline-flex h-11 items-center gap-2 rounded-xl bg-graf-950 px-5 text-sm font-extrabold text-white hover:bg-graf-800">Continuar <ArrowRight className="size-4" aria-hidden /></button>
          ) : (
            <button disabled={pendente} className="inline-flex h-11 items-center gap-2 rounded-xl bg-jb-600 px-5 text-sm font-extrabold text-white hover:bg-jb-500 disabled:cursor-wait disabled:opacity-60">{pendente ? "Enviando…" : "Enviar chamado"}<Wrench className="size-4" aria-hidden /></button>
          )}
        </div>
      </div>
    </form>
