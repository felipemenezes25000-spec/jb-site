 ...
 *
 * Mobile e desktop são auditados separadamente: cabeçalho, navegação e barra
 * fixa mudam de composição conforme a largura, então um único viewport não é
 * evidência suficiente para os dois produtos visuais.
 */
      ...
      try {
        ...
        if (!resposta || resposta.status() >= 400) {
          problemas.push({
            ...
            tipo: "http",
            detalhe: `HTTP ${resposta?.status() ?? "sem resposta"}`,
          });
          continue;
        }
        ...
        // com alvo confortável. O mínimo de 44px é cobrado nas superfícies de
        // conversão, inclusive no desktop quando o mesmo botão continua visível.
        ...
        for (const detalhe of extras) {
          problemas.push({ rota, cenario: cenario.nome, tipo: "medicao-propria", detalhe });
        }
...
console.log(`\nAcessibilidade: ${ROTAS.length} rotas × ${CENARIOS.length} cenários sem violações no gate.\n`);
